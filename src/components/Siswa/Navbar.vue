...
const notif = () =>
{
    toast.add({ severity: toastType, summary: 'Info', detail: toastMessage , life: 2000, styleClass : 'max-w-[22rem]', group : 'tc1' });
}
...
const logoutConfirm = () =>
{
    confirm.require({
        group : 'headless',
        message: 'Ingin logout dari aplikasi ? Anda hanya bisa login kembali setelah 24 jam',
        header: 'Peringatan',
    ...
    <div class="max-h-[4rem] h-[4rem] justify-between text-sm bg-sky-500 p-4 w-full flex items-center text-gray-700 mb-2 rounded-lg">
        ...
        <Drawer v-model:visible="showDrawer" header="Menu" class="!w-[50%]" close-icon>
            <div class="flex flex-col justify-center gap-y-8">
                <NavbarLink :to="{name : 'siswaDashboard'}" :active=" currentRoute == 'siswaDashboard'">
                    Dashboard
                </NavbarLink>
                <!-- <NavbarLink :to="{name : 'siswaAbsensiIndex'}" :active=" currentRoute == 'siswaAbsensiIndex'">
                    Absensi
                </NavbarLink> -->
                <NavbarLink :to="{name : 'siswaJadwalIndex'}" :active=" currentRoute == 'siswaJadwalIndex'">
                    Jadwal
                </NavbarLink>
            </div>
        </Drawer>
